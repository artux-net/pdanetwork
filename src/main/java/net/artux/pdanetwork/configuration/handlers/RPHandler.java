...
@Service
public class RPHandler extends SocketHandler {
    ...
    public RPHandler(ValuesService valuesService, UserService userService, ObjectMapper objectMapper, MessageMapper messageMapper) {
    ...
    public void afterConnectionEstablished(WebSocketSession userSession) {
        ...
        if (getMember(userSession).isChatBan())
            ...
            sendSystemMessage(userSession, "Вы подключены к RP - чату. <a href=\"" + valuesService.getAddress() + "/rules\">Соблюдайте правила.</a>");
    }
